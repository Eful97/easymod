...
async function buildIndexBundle(minify = false) {
    ...
    try {
        await esbuild.build({
            ...
            target: ['es2016'],
            format: 'cjs',
                external: ['cheerio', 'cheerio-select', 'undici', 'fs', 'path', 'https', 'http', 'http2', 'url', 'crypto', 'util', 'zlib', 'stream', 'buffer', 'events', 'assert', 'sql.js', 'puppeteer-extra', 'puppeteer-extra-plugin-stealth', 'form-data', 'axios', 'child_process'],
            define: {
    ...
    for (const file of files) {
        ...
        try {
            const result = await esbuild.build({
                ...
                format: 'cjs',
                external: ['undici', 'fs', 'path', 'https', 'http', 'http2', 'url', 'crypto', 'util', 'zlib', 'stream', 'buffer', 'events', 'assert', 'sql.js', 'puppeteer-extra', 'puppeteer-extra-plugin-stealth', 'form-data', 'axios'],
                define: {
                    'process.env.NODE_ENV': minify ? '"production"' : '"development"'
                }
            });
    ...
    for (const provider of providers) {
        ...
        try {
            await esbuild.build({
                ...
                external: ['undici', 'fs', 'path', 'https', 'http', 'http2', 'url', 'crypto', 'util', 'zlib', 'stream', 'buffer', 'events', 'assert', 'sql.js', 'puppeteer-extra', 'puppeteer-extra-plugin-stealth', 'axios', 'child_process']
            });
