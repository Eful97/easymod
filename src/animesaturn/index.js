...
function normalizeAnimeSaturnSearchText(value) {
  return stripHtmlTags(value)
    .toLowerCase()
    .replace(/\b(?:sub\s*ita|sub|ita|dub|dubbed|streaming|animesaturn)\b/g, " ")
    .replace(/[^a-z0-9]+/g, " ")
    .replace(/\s{2,}/g, " ")
    .trim();
}
...
function splitAnimeSaturnSearchTokens(value) {
  ...
  return normalizeAnimeSaturnSearchText(value)
    .split(/\s+/)
    .filter((token) => token.length > 1 && !stopWords.has(token));
}

function buildAnimeSaturnRecordPath(record) {
  const link = String(record?.link || record?.path || "").trim().replace(/^\/+|\/+$/g, "");
...
function detectExplicitAnimeSaturnSeason(record) {
  const text = normalizeAnimeSaturnSearchText(`${record?.name || ""} ${record?.link || ""}`);
  const link = String(record?.link || "").toLowerCase();
...
function scoreAnimeSaturnSearchRecord(record, titleCandidates, requestedSeason) {
  ...
  const haystack = normalizeAnimeSaturnSearchText(`${record?.name || ""} ${record?.link || ""}`);
  ...
  for (const candidate of titleCandidates) {
    ...
    const recordName = normalizeAnimeSaturnSearchText(record?.name || "");
    if (needle.includes(recordName)) score += 20;
    const recordTokens = splitAnimeSaturnSearchTokens(record?.name || record?.link || "");
...
function selectAnimeSaturnSearchPaths(records, titleCandidates, requestedSeason) {
  const scored = (Array.isArray(records) ? records : [])
    .map((record) => scoreAnimeSaturnSearchRecord(record, titleCandidates, requestedSeason))
    .filter(Boolean)
...
async function fetchAnimeSaturnSearchRecords(title) {
...
async function resolveAnimeSaturnPathsByTitle(lookup, mappingPayload, providerContext = null) {
...
async function getStreams(id, type, season, episode, providerContext = null) {
  try {
    ...
    if (animePaths.length === 0) {
      const titleFallbackPaths = await resolveAnimeSaturnPathsByTitle(
        lookup,
        mappingPayload,
        providerContext
      );
